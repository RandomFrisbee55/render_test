            ...
            for row in reader:
                try:
                    module = {
                        ...
                        "link": row.get("Link", "")  # Read the single Link column
                    }
...
def hierarchical_ranking(user_preferences, all_modules):
    ...
    return [(m["name"], m["primary_score"], m["features"], m["link"]) for m in module_scores[:5]]
...
def recommend_modules(user_vector, user_preferences, all_modules):
    ...
    for module in all_modules:
        ...
        euclidean_scores.append((module["name"], euclidean_score, module["features"], module["link"]))
...
async def submit_form(
    ...
    for interest in user_niche_interests:
        if interest in special_topic_modules:
            ...
            # Find the module in all_modules to get its link
...
# Load modules from CSV (unchanged)
def load_modules(csv_file_path):
    all_modules = []
    try:
        with open(csv_file_path, newline='') as csvfile:
...
# Load modules at startup (unchanged)
csv_path = os.path.join(os.path.dirname(__file__), "modules3.csv")
all_modules = load_modules(csv_path)
if not all_modules:
    print("Warning: No modules loaded from CSV")
...
async def submit_form(
    ...
    for interest in user_niche_interests:
        if interest in special_topic_modules:
            recommended_niche_modules.append(special_topic_modules[interest])
    ...
    return templates.TemplateResponse(
        "thankyou.html",
        {
            ...
            "euclidean_recommendations": top_5_euclidean,
            "hierarchical_recommendations": top_5_hierarchical,
            "niche_recommendations": recommended_niche_modules  # New field
        }
    )"""
